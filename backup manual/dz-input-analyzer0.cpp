...
{
    // Defaults must be set here so OBS doesn't treat missing values as 0
    // and make the source fully transparent/invisible on first load.
    obs_data_set_int(settings, "width", 700);
    obs_data_set_int(settings, "height", 180);
    obs_data_set_double(settings, "bg_alpha", 0.65);
    obs_data_set_double(settings, "font_size", 14.0);
    obs_data_set_double(settings, "scale", 1.0);
}
...
static void dz_source_render(void *data, gs_effect_t *effect)
{
	...
	for (const auto &c : d->clicks) {
		...
		dz_draw_text_5x7(solid, x + 6.0f, yText + 22.0f, buf, scale, num);
	}
	...
	// TIME axis line + ticks + labels 0s..5s
	{
		...
		for (int i = 0; i <= 5; i++) {
			...
			dz_draw_text_5x7(solid, x - 10.0f, axisY + 10.0f, lab, 3.0f, tcol);
		}
	}
