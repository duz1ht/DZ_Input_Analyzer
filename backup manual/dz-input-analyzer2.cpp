...
// ------------------------------------------------------------
// Intrinsic layout sizing (tight Source Bounding Box)
//
// OBS uses get_width/get_height as the source's base size. That base size drives the
// Source Bounding Box and Transform operations like "Fit to Screen".
// We keep it tightly matched to what we draw and let users scale via Transform.
static const int64_t kTimelineWindowMs = 5000; // 5 seconds shown

static const float kLeftPad = 70.0f;
static const float kRightPad = 20.0f;
static const float kTopPad = 18.0f;
static const float kBottomPad = 55.0f;

static const float kRowGap = 20.0f;
static const float kRowH = 44.0f;

static const float kTimelineW = 700.0f; // base width for the 5s window

static inline void dz_compute_intrinsic_size(uint32_t &out_w, uint32_t &out_h)
{
	const float W = kLeftPad + kTimelineW + kRightPad;
	const float H = kTopPad + kBottomPad + (kRowH * (float)ROW_COUNT) + (kRowGap * (float)(ROW_COUNT - 1));

	out_w = (uint32_t)std::max(16.0f, std::floor(W + 0.5f));
	out_h = (uint32_t)std::max(16.0f, std::floor(H + 0.5f));
}
...
struct dz_source_data {
	// Source size (tight bounding box)
	uint32_t width = 0;
	uint32_t height = 0;
...
// Minimal bitmap font (5x7) for: W S A D T I M E 0..5 's'
static uint8_t glyph_5x7(char ch, int row)
{
	...
	case 'I': {
		static const uint8_t g[7] = {0b11111, 0b00100, 0b00100, 0b00100, 0b00100, 0b00100, 0b11111};
		return g[row];
	}
	case 'M': {
		static const uint8_t g[7] = {0b10001, 0b11011, 0b10101, 0b10001, 0b10001, 0b10001, 0b10001};
		return g[row];
	}
	case 'E': {
...
static void dz_draw_vline(gs_effect_t *solid, float x, float y0, float y1, float w, const vec4 &c)
{
	const float h = std::max(1.0f, y1 - y0);
	dz_draw_rect(solid, x, y0, w, h, c);
}
...
{
	obs_data_set_default_double(settings, "bg_alpha", 0.65);
}
...
static void *dz_source_create(obs_data_t *settings, obs_source_t *source)
{
	...
	// Intrinsic size: keep Source Bounding Box tight.
	dz_compute_intrinsic_size(d->width, d->height);
...
static void dz_source_update(void *data, obs_data_t *settings)
{
	...
	// Keep Source Bounding Box tight.
	dz_compute_intrinsic_size(d->width, d->height);

	if (obs_data_has_user_value(settings, "bg_alpha")) {
		d->bg_alpha = (float)obs_data_get_double(settings, "bg_alpha");
		d->bg_alpha = std::clamp(d->bg_alpha, 0.0f, 1.0f);
	}

	blog(LOG_INFO, "[dz-input-analyzer] update: intrinsic=%ux%u bg_alpha=%.2f", d->width, d->height, d->bg_alpha);
}
...
static void dz_source_render(void *data, gs_effect_t *effect)
{
	...
	// Ensure base size stays tight to what we draw.
	uint32_t iw = 0, ih = 0;
	dz_compute_intrinsic_size(iw, ih);
	if (d->width != iw)
		d->width = iw;
	if (d->height != ih)
		d->height = ih;
	...
	// Ensure our draw coordinates map 1:1 to the source base size.
	gs_viewport_push();
	...
	const float leftPad = kLeftPad;
	const float rightPad = kRightPad;
	const float topPad = kTopPad;
	const float bottomPad = kBottomPad;
	...
	const int64_t WINDOW_MS = kTimelineWindowMs;
	...
	vec4 grid = dz_col_rgba(1.0f, 1.0f, 1.0f, 0.16f);
	for (int i = 0; i <= 5; i++) {
		const float x = timelineX0 + ((float)i * 1000.0f / (float)WINDOW_MS) * timelineW;
		dz_draw_vline(solid, x, topPad - 6.0f, H - bottomPad + 8.0f, 1.0f, grid);
	}
	...
	for (const auto &c : d->clicks) {
		...
		vec4 marker = dz_col_rgba(1.0f, 1.0f, 1.0f, 0.18f);
		dz_draw_vline(solid, x, topPad - 6.0f, H - bottomPad + 8.0f, 1.0f, marker);

		// Number near the row
		vec4 num = dz_col_rgba(1.0f, 1.0f, 1.0f, 0.80f);
		const float scale = 3.0f;

		// Convert delta to string (max 5 digits drawn nicely)
		char buf[16]{};
		_snprintf_s(buf, _TRUNCATE, "%d", c.delta_ms);

		const float yText = rowYs[c.row] - 6.0f;
		float textX = x + 6.0f;
		// Keep text inside bounds so intrinsic bbox stays tight.
		const float char_w = 6.0f * scale;
		const float text_w = (float)strlen(buf) * char_w;
		if (textX + text_w > W)
			textX = W - text_w;
		if (textX < 0.0f)
			textX = 0.0f;
		dz_draw_text_5x7(solid, textX, yText + 22.0f, buf, scale, num);
	}
	...
	// TIME axis line + ticks + labels 0s..5s
	{
		...
		vec4 axis = dz_col_rgba(1.0f, 1.0f, 1.0f, 0.28f);
		dz_draw_rect(solid, timelineX0, axisY, timelineW, 2.0f, axis);

		// "TIME" label
		vec4 tcol = dz_col_rgba(1.0f, 1.0f, 1.0f, 0.75f);
		dz_draw_text_5x7(solid, 18.0f, axisY - 4.0f, "TIME", 3.0f, tcol);

		for (int i = 0; i <= 5; i++) {
			const float x = timelineX0 + ((float)i * 1000.0f / (float)WINDOW_MS) * timelineW;
			dz_draw_rect(solid, x, axisY, 2.0f, 12.0f, axis);
			...
			const float scale = 3.0f;
			const float char_w = 6.0f * scale;
			const float text_w = (float)strlen(lab) * char_w;
			if (labelX < 0.0f)
	...
	gs_viewport_pop();
			...
			if (labelX + text_w > W)
				labelX = W - text_w;
			dz_draw_text_5x7(solid, labelX, axisY + 10.0f, lab, scale, tcol);
		}
	}
